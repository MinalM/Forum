...
describe('"Related questions" links (PostDetail) are at least 44px tall', () => {
  it('declares a min-height of 44px on the related-questions-item link', () => {
    const rule = extractRule(appCss, '.related-questions-item a');
    expect(minPx(rule, 'min-height')).toBeGreaterThanOrEqual(MIN_TARGET);
  });
});
