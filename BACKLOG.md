...
- [ ] **The post view's tag chips and action controls have no `gap` — wrapped
  tag rows touch, and the tag block sits flush on the action row.** Reported by
  the maintainer ("the buttons/tags are very close to each other") and measured
  on a local run at 1280px and 375px, logged in as admin. Three separate
  spacings, all zero:
  1. `.post-tags` (`client/src/App.css:367`) is `display: flex; flex-wrap: wrap`
     with **no `gap`**. Its horizontal spacing comes entirely from the legacy
     `.badge { margin-right: 0.5rem }` in `client/src/index.css`, which does
     nothing between *rows* — a post with 8 tags renders 2 rows whose measured
     **row gap is 0px**, so the chips read as one solid purple block. The
     margin also leaves a dangling 8px after the last chip in each row.
  2. `.post-tags` has no bottom margin, so the measured vertical gap from the
     tag block to `.post-actions` is **0px** at both widths.
  3. `.comment-actions` (`App.css:830`) is `display: flex` with no `gap`;
     ...
     apart.
  Scope: `client/src/App.css`, client-only, no markup or route changes needed
  beyond removing the margin-based spacing. Put a real `gap` on each of these
  flex containers (`.post-actions` already uses `gap: 0.75rem` — follow it),
  give `.post-tags` a row gap so wrapped rows breathe and a bottom margin so it
  separates from the action row, and stop relying on `.badge`'s `margin-right`
  for spacing inside a flex row. Check the other places `.badge` is used inside
  a flex container while you are there (`PostItem`'s tag row is the obvious
  sibling) so the two surfaces stay consistent.
  Note the general rule this violates, which is worth not reintroducing:
  margin-based spacing does not survive wrapping or reordering, `gap` does.
  Acceptance: a raw-CSS assertion that `.post-tags`, `.comment-actions` and the
  feed card's tag row each declare a non-zero `gap` and that none of them
  depends on `.badge`'s `margin-right` for separation; a jsdom/rendered check on
  a post with enough tags to wrap asserts a non-zero vertical gap between tag
  rows and between the tag block and `.post-actions`; existing `PostDetail` and
